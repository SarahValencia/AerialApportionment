---
title: "Using Aerial Apportionment to Assess Community Composition Near TRI Facilities: Long Island"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using Aerial Apportionment to Assess Community Composition Near TRI Facilities: Long Island}
...
First we set our Census API key:
...
Now we get 450 variables, but by scrolling through I can see that the term I'm looking for is median household income. So we try again with this term (and note that capitalization doesn't matter, but I just copied and pasted this from the table).
...
I get a much narrower set of variables, and I see that code B19013_001 corresponds with the median household income that is not broken down by age. 

After running this function multiple times I've assembled a list of variables I would like to track, namely, the number of people of each race/ethnicity and the median household income. In the PullCensusData function we can either call census variables directly by their codes, or we can create a named vector of variable names so that the headings that appear in the table are more informative to the user. 
...
# The RSEISearchFunction is very flexible, with lots of optional arguments to allows users to narrow in on data from the location, time period, industry sector, or facilities they are interested in. Here, we are interested in facilities in the 5 years between 2017 and 2021 (the most recent year of data), in the four counties on Long Island. We want to project the facility locations into the metric WGS 84 coordinate system so that we can overlay facility locations on our census block data, and I want to save the facilty location data and the RSEI data to myworkingdirectory.

```{r}
LIFacilities <- RSEISearch(DownloadRSEI= F, RSEIpath = paste(myworkingdirectory,"RSEI_Public_Release_Data_V2311_RY2021",sep="/"), RSEIversion="v2311", StartYr=2017, EndYr=2021, State = "NY", County = LICounties, returnLocation = T, projectTo = 3857, wider=TRUE, Filename = "RSEI_LIFacilities", SaveTo = myworkingdirectory)
```

We can save the 2 objects in the list as dataframes.

```{r}
LIFacilitiesEmissions <- LIFacilities[[1]]
glimpse(LIFacilitiesEmissions)
str(LIFacilitiesEmissions)


LI_Facility_locations <- LIFacilities[[2]]
str(LI_Facility_locations)
st_crs(LI_Facility_locations)
```
...
```{r}
setwd(paste(myworkingdirectory,"RSEI_Public_Release_Data_V2311_RY2021",sep="/"))
Facility <- read.csv("facility_data_rsei_v2311.csv") # has all Facility info
...
```{r}
LIleadSearch_1 <- RSEISearch(DownloadRSEI= F, RSEIpath = paste(myworkingdirectory,"RSEI_Public_Release_Data_V2311_RY2021",sep="/"), RSEIversion="v2311", StartYr=2017, EndYr=2021, State = "NY", County = LICounties, rsei_Chem_Numbers  = c(346,347), Filename = "Lead1")
head(LIleadSearch_1)

LIleadSearch_2 <- RSEISearch(DownloadRSEI= F, RSEIpath = paste(myworkingdirectory,"RSEI_Public_Release_Data_V2311_RY2021",sep="/"), RSEIversion="v2311", StartYr=2017, EndYr=2021, State = "NY", County = LICounties, ChemicalNames = "lead",  Filename = "Lead2")
head(LIleadSearch_2)
```

## Calculating Community Characteristics via Aerial Apportionment Method
